...
def run_gui():
    ...
    def abrir_asignacion_correos(comunidades):
        ...
        # Frame para la tabla dentro del frame desplazable
        ...
        # Frame para los botones fuera del área de desplazamiento
        btn_frame = tk.Frame(win)
        btn_frame.pack(fill='x', pady=10)
        
        btn_guardar = ttk.Button(btn_frame, text='Guardar y continuar', command=lambda: guardar_y_continuar_wrapper())
    ...
    try:
        ...
        def procesar_en_hilo_wrapper(): # Wrapper to avoid issues with procesando/detener_proceso scope if run_gui is called multiple times
            ...
            try:
                ...
                if not carpeta or not remitente: # Gmail pass can be empty for testing GUI
                    messagebox.showerror("Error", "Por favor complete la carpeta y el correo remitente.")
                    procesando = False
                    return
