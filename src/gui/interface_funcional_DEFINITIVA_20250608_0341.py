...
def run_gui():
    ...
    try:
        ...
        # Variables para la barra de progreso y etiquetas
        progress_label_text_var = tk.StringVar(value="Progreso: N/A")
        time_label_text_var = tk.StringVar(value="Tiempo restante: N/A")
        # progress_bar será definido más adelante con los widgets
        ...
        def abrir_ventana_asignacion_correos(lista_comunidades_procesadas):
            ...
            def send_selected_action():
                ...
                if not items_para_enviar_raw:
                    ...
                    # Aunque no haya items para enviar, el resumen se mostrará, así que las variables deben estar definidas.
                    # El código del resumen se ejecutará después de este bloque if.
                    # No es necesario un 'return' aquí si queremos mostrar siempre el messagebox de resumen.
                    # Sin embargo, el comportamiento actual es retornar, lo cual está bien.
                    return

                log_func_hilo(f"Iniciando proceso de envío real para {len(items_para_enviar_raw)} factura(s) individual(es).")
                
                for item_a_enviar in items_para_enviar_raw:
                    destinatario_email = item_a_enviar.get('correo_asignado').strip()
                    nombre_comunidad_actual = item_a_enviar.get('nombre_comunidad', 'N/A')
                    ruta_pdf_actual = item_a_enviar.get('ruta_pdf', 'N/A')

                    if ruta_pdf_actual == 'N/A' or not os.path.exists(ruta_pdf_actual):
                        log_func_hilo(f"ADVERTENCIA: PDF no válido o no encontrado para {nombre_comunidad_actual} ({ruta_pdf_actual}). Saltando envío para esta factura.")
                        errores_count += 1
                        detalles_errores.append(f"Comunidad {nombre_comunidad_actual}: PDF no encontrado en {ruta_pdf_actual}.")
                        continue

                    # Cada correo es para una sola comunidad/PDF
                    asunto_email = f"Factura Comunidad: {nombre_comunidad_actual}"
                    cuerpo_email = f"Estimado/a cliente,\n\nAdjunto le remitimos su factura correspondiente a la comunidad: {nombre_comunidad_actual}.\n\nPara cualquier consulta, no dude en contactarnos."
                    # El texto legal se añade automáticamente dentro de enviar_correo

                    log_func_hilo(f"Intentando enviar a: {destinatario_email} el PDF para {nombre_comunidad_actual} ({os.path.basename(ruta_pdf_actual)}).")
                    
                    try:
                        # enviar_correo espera una lista de PDFs, así que envolvemos la ruta_pdf_actual en una lista
                        enviar_correo(remitente_email, remitente_password, destinatario_email, asunto_email, cuerpo_email, [ruta_pdf_actual])
                        log_func_hilo(f"Correo enviado exitosamente a {destinatario_email} para {nombre_comunidad_actual}.")
                        enviados_count += 1
                    except Exception as e_send:
                        log_func_hilo(f"ERROR al enviar correo a {destinatario_email} para {nombre_comunidad_actual}: {str(e_send)}")
                        errores_count += 1
                        detalles_errores.append(f"Destinatario {destinatario_email} (Comunidad: {nombre_comunidad_actual}): {str(e_send)}")
                # Construir el mensaje de resumen después del bucle
        ...
        # --- Frame para el estado del progreso ---
        progress_status_frame = tk.Frame(main_frame)
        progress_status_frame.pack(fill='x', pady=(5, 0), after=button_frame)

        global progress_bar
        
        progress_label = ttk.Label(progress_status_frame, textvariable=progress_label_text_var)
        progress_label.pack(side='top', fill='x', padx=2)

        progress_bar = ttk.Progressbar(progress_status_frame, orient='horizontal', mode='determinate', length=300)
        progress_bar.pack(side='top', fill='x', padx=2, pady=(2, 5))

        time_label = ttk.Label(progress_status_frame, textvariable=time_label_text_var)
        time_label.pack(side='top', fill='x', padx=2)
        # --- Fin Frame para el estado del progreso ---
        ...
        def iniciar_proceso_wrapper():
            ...
            try:
                ...
                if not archivos_pdf:
                    ...
                    root.after(0, lambda: progress_label_text_var.set("Progreso: 0 PDFs encontrados"))
                    root.after(0, lambda: time_label_text_var.set("Tiempo restante: N/A"))
                    procesando = False
                ...
                root.after(0, lambda: progress_bar.config(value=0))
                root.after(0, lambda: progress_label_text_var.set(f"Procesados: 0/{total_pdfs} (0.0%)" if total_pdfs > 0 else "Progreso: 0 PDFs"))
                root.after(0, lambda: time_label_text_var.set("Tiempo restante: Calculando..." if total_pdfs > 0 else "Tiempo restante: N/A"))
                ...
                for idx, pdf_path in enumerate(archivos_pdf):
                    ...
                    if parada_evento.is_set(): # Check if the stop event has been signaled
                        ...
                        # Update progress display for stop
                        processed_on_stop = idx
                        percentage_on_stop = (processed_on_stop / total_pdfs) * 100 if total_pdfs > 0 else 0
                        
                        val_p_stop = percentage_on_stop
                        val_proc_stop = processed_on_stop
                        val_tot_stop = total_pdfs

                        root.after(0, lambda p=val_p_stop, pd_c=val_proc_stop, tot_pd=val_tot_stop: (
                            progress_bar.config(value=p),
                            progress_label_text_var.set(f"Detenido. Procesados: {pd_c}/{tot_pd} ({p:.1f}%)" if tot_pd > 0 else f"Detenido. Procesados: {pd_c}/{tot_pd}"),
                            time_label_text_var.set("Tiempo restante: Detenido")
                        ))
                        break
                    ...
                    # --- Progress Update ---
                    processed_pdfs_loop = idx + 1
                    percentage_loop = (processed_pdfs_loop / total_pdfs) * 100 if total_pdfs > 0 else 0
                    
                    current_loop_time = time.time()
                    elapsed_time_loop = current_loop_time - start_time
                    
                    est_remaining_time_str_loop = "Calculando..."
                    if total_pdfs > 0 and processed_pdfs_loop > 0:
                        time_per_pdf_loop = elapsed_time_loop / processed_pdfs_loop
                        remaining_pdfs_loop = total_pdfs - processed_pdfs_loop
                        estimated_remaining_seconds_loop = remaining_pdfs_loop * time_per_pdf_loop
                        est_remaining_time_str_loop = format_time_seconds(estimated_remaining_seconds_loop)
                    elif total_pdfs == 0:
                        est_remaining_time_str_loop = "N/A"

                    val_p = percentage_loop
                    val_proc = processed_pdfs_loop
                    val_tot = total_pdfs
                    val_ert = est_remaining_time_str_loop

                    root.after(0, lambda p=val_p, pd_c=val_proc, tot_pd=val_tot, ert_s=val_ert: (
                        progress_bar.config(value=p),
                        progress_label_text_var.set(f"Procesados: {pd_c}/{tot_pd} ({p:.1f}%)" if tot_pd > 0 else f"Procesados: {pd_c}/{tot_pd}"),
                        time_label_text_var.set(f"Tiempo restante: {ert_s}")
                    ))
                    # --- End Progress Update ---
                ...
                # Final progress update if loop completed
                if not parada_evento.is_set() and total_pdfs > 0:
                    final_elapsed_time = time.time() - start_time
                    root.after(0, lambda: progress_bar.config(value=100))
                    root.after(0, lambda: progress_label_text_var.set(f"Completado: {total_pdfs}/{total_pdfs} (100.0%)"))
                    root.after(0, lambda t=final_elapsed_time: time_label_text_var.set(f"Tiempo total: {format_time_seconds(t)}"))
                elif not parada_evento.is_set() and total_pdfs == 0:
                    root.after(0, lambda: progress_bar.config(value=0))
                    root.after(0, lambda: progress_label_text_var.set("Progreso: 0 PDFs procesados"))
                    root.after(0, lambda: time_label_text_var.set("Tiempo restante: N/A"))
                ...
                current_progress_val = progress_bar['value'] if 'progress_bar' in globals() and progress_bar.winfo_exists() else 0
                root.after(0, lambda val=current_progress_val: progress_bar.config(value=val))
                root.after(0, lambda: progress_label_text_var.set("Error durante el procesamiento. Ver logs."))
                root.after(0, lambda: time_label_text_var.set("Tiempo restante: Error"))
                procesando = False
